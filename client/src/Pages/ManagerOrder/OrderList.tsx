import { useEffect, useState } from "react";
import React from "react";
import BaseAxios from "../../api/axiosClient";
import Paginationn from "../../components/common/User/Footer/Pagination";
import { useSelector } from "react-redux";
import { DialogCustomAnimationn } from "./OrderItem";
import moment from "moment";
import { CSVLink } from "react-csv";

const OrderList = () => {
  const page = useSelector((state: any) => state.pagination);
  const [orderList, setOrderList] = useState<any[]>([]);
  const [orderListWithOrderDate, setOrderListWithOrderDate] = useState<any[]>(
    []
  );
  const [orderMap, setOrderMap] = useState<any[]>([]);
  const [orderLists, setOrderLists] = useState<any[]>([]);
  const [totalPage, setTotalPage] = useState();
  const [searchInput, setSearchInput] = useState("");
  const [sortOrder, setSortOrder] = useState("asc");
  const handleSortOrder = () => {
    setSortOrder(sortOrder === "asc" ? "desc" : "asc");
  };
  const handleGetDataa: () => void = () => {
    BaseAxios.get(`/api/v1/orders?page=${page}`)
      .then((response) => {
        setTotalPage(response.data.totalPage);
        setOrderList(response.data.result);
      })
      .catch((error) => console.log(error));
    BaseAxios.get(`/api/v1/orders`)
      .then((response) => {
        setOrderLists(response.data);
      })
      .catch((error) => console.log(error));
  };
  async function handleChangeStatus(
    e: React.ChangeEvent<HTMLSelectElement>,
    id: any
  ) {
    const selectedStatus = e.target.value;

    // Kiểm tra nếu status là 'Completed', không cho phép thay đổi trạng thái
    if (selectedStatus === "Completed") {
      alert("Cannot change status of completed order");
      return;
    }

    // Tiến hành gửi yêu cầu chỉnh sửa status đến server
    await BaseAxios.put(`/api/v1/orders/statusOrder/${id}`, {
      status: selectedStatus,
    });
    alert("Order updated successfully");
  }
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    setSearchInput(e.target.value);
  }
  useEffect(() => {
    const searchResult = orderList?.filter((item) => {
      return item?.codeOrder?.toLowerCase().includes(searchInput);
    });
    if (searchInput?.length === 0) {
      handleGetDataa();
    } else {
      setOrderList(searchResult);
    }
  }, [searchInput, page]);
  useEffect(() => {
    BaseAxios.get("/api/v1/orders/orderdate/order/orderwwithdate/1")
      .then((res) => {
        setOrderListWithOrderDate(res.data);
      })
      .catch((err) => console.log(err));
    const updatedOrderMap =
      sortOrder === "asc" ? orderList : orderListWithOrderDate;
    setOrderMap(updatedOrderMap);
  }, [sortOrder, orderMap]);

  return (
    <>
      <div className="col-span-1 overflow-auto bg-white shadow-lg p-4">
        <div className="pb-4 bg-white dark:bg-gray-900 search ">
          {/* <button className="text-bg-white-800 bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2  dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">
            <CSVLink
              data={orderLists}
              filename={"order_list.csv"}
              className="btn-export-file"
            >
              Export file
            </CSVLink>
          </button> */}

          <label htmlFor="table-search" className="sr-only ">
            Search
          </label>

          <div className="relative mt-1 ml-30px">
            <div className="absolute inset-y-0  left-0 flex items-center pl-3 pointer-events-none">
              <svg
                className="w-4 h-4 text-gray-500 dark:text-gray-400"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 20 20"
              >
                <path
                  stroke="currentColor"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"
                />
              </svg>
            </div>
            <input
              onChange={handleChange}
              type="text"
              id="table-search"
              className="block p-2 pl-10 text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder="Search"
            />
          </div>
        </div>
        <table className="table-auto border w-full my-2">
          <thead>
            <tr>
              <th className="px-4 py-2 border">ID</th>
              <th className="px-4 py-2 border">Code Order</th>
              <th className="px-4 py-2 border">Date</th>
              <th className="px-4 py-2 border">Total</th>
              <th className="px-4 py-2 border">Status</th>
              <th className="px-4 py-2 border">Detail</th>
            </tr>
          </thead>
          <tbody>
            {sortOrder == "asc"
              ? orderList?.map((order, index) => (
                  <tr
                    key={order?.id}
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                  >
                    <th
                      scope="row"
                      className="px-4 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"
                    >
                      {index + 1}
                    </th>
                    <td className="px-3 py-4 text-center">
                      {order?.codeOrder}
                    </td>
                    <td className="px-3 py-4 text-center">
                      {moment(order?.orderDate).format("lll")}
                    </td>
                    <td className="px-3 py-4 text-center">
                      {order?.totalAmount.toLocaleString()} VND
                    </td>
                    <td className="px-3 py-4 text-center">
                      <select
                        style={{
                          height: "26px",
                          border: "1px solid #ccc",
                          borderRadius: "3px",
                          padding: "0 0 0 13px",
                          fontSize: "13px",
                          width: "100px",
                        }}
                        onChange={(e) => handleChangeStatus(e, order?.id)}
                        name="status"
                        id="status"
                        required
                        disabled={order?.status?.toLowerCase() === "cancelled"}
                      >
                        <option value="">{order?.status}</option>
                        <option value="Pending">Pending</option>
                        <option value="Processing">Processing</option>
                        <option value="Shipping">Shipping</option>
                        <option value="Completed">Completed</option>
                      </select>
                    </td>
                    <td className="px-3 py-4 text-center">
                      <button className="btn-detail-order-admin">
                        <DialogCustomAnimationn idOrder={order?.id} />
                      </button>
                    </td>
                  </tr>
                ))
              : orderMap?.map((order, index) => (
                  <tr
                    key={order?.id}
                    className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                  >
                    <th
                      scope="row"
                      className="px-4 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"
                    >
                      {index + 1}
                    </th>
                    <td className="px-3 py-4">{order?.id}</td>
                    <td className="px-3 py-4">
                      {order?.orderDate?.toDateString()}
                    </td>
                    <td className="px-3 py-4">
                      {order?.totalAmount?.toLocaleString()} VND
                    </td>
                    <td className="px-3 py-4">
                      <select
                        style={{
                          height: "26px",
                          border: "1px solid #ccc",
                          borderRadius: "3px",
                          padding: "0 0 0 13px",
                          fontSize: "13px",
                          width: "100px",
                        }}
                        onChange={(e) => handleChangeStatus(e, order?.id)}
                        name="status"
                        id="status"
                        required
                        disabled={order?.status?.toLowerCase() === "cancelled"}
                      >
                        <option value="">{order?.status}</option>
                        <option value="Pending">Pending</option>
                        <option value="Processing">Processing</option>
                        <option value="Shipping">Shipping</option>
                        <option value="Completed">Completed</option>
                      </select>
                    </td>
                    <td className="px-3 py-4">
                      <button className="btn-detail-order-admin text-bg-white-800 bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2  dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">
                        <DialogCustomAnimationn idOrder={order?.id} />
                      </button>
                    </td>
                  </tr>
                ))}
          </tbody>
        </table>
      </div>
      <div style={{ marginTop: "5%" }}>
        {" "}
        <Paginationn totalPage={totalPage} />
      </div>
    </>
  );
};

export default OrderList;
